...
/**
 * useDelete Hook - DELETE istekleri için
 * Veri silme işlemleri için kullanılır
 *
 * @example
 * const { mutate, loading } = useDelete<void, number>((id) => `/users/${id}`);
 * mutate(123);
 */
...
/**
 * useDeleteById Hook - ID ile silme için özelleştirilmiş hook
 *
 * @example
 * const { deleteById, loading } = useDeleteById<void>('/users');
 * deleteById(123);
 */
